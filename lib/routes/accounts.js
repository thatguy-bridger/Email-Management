...
// Logic behind api/accounts/index.js, which dispatches list/create/test/
// update/delete/sync on ?id=/?action= in the query string. Kept here
// rather than inline so the route file itself stays a thin dispatcher.
...
export const PUBLIC_COLUMNS = `
  ...
  backfill_complete, backfill_total_estimate
`;

export async function listAccounts(user, req, res) {
  // Prefixed rather than reusing PUBLIC_COLUMNS unprefixed -- the join below
  // introduces a second table with its own `id`, which would otherwise be
  // ambiguous.
  const prefixedColumns = PUBLIC_COLUMNS.split(',')
    .map((c) => `a.${c.trim()}`)
    .join(', ');
  const { rows } = await query(
    `SELECT ${prefixedColumns}, COALESCE(mc.count, 0)::int AS messages_synced
     FROM accounts a

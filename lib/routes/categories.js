...
export async function createCategory(user, req, res) {
  const { name, color, icon, groupName } = req.body || {};
  ...
  const id = newId('cat');
  const { rows } = await query(
    `INSERT INTO categories (id, user_id, name, color, icon, is_builtin, sort_order, group_name)
     VALUES ($1,$2,$3,$4,$5,false,$6,$7) RETURNING *`,
    [id, user.id, name, color || '#6366f1', icon || 'tag', maxRows[0].next, groupName || null]
  );
...
export async function updateOrDeleteCategory(user, id, req, res) {
  ...
  const { name, color, icon, sortOrder, groupName } = req.body || {};
  const { rows } = await query(
    `UPDATE categories SET
       ...
       group_name = CASE WHEN $7 THEN $8 ELSE group_name END
     WHERE id = $1 AND user_id = $2 RETURNING *`,
    // node-postgres rejects `undefined` params, so omitted fields need to
    // become null before hitting COALESCE. group_name is a genuine
    // nullable field (clearing a category's group is a valid edit), so it
    // can't use the same COALESCE-skips-null trick as the others -- a
    // separate "was this field provided at all" flag distinguishes
    // "leave unchanged" from "set to ungrouped".
    [id, user.id, name ?? null, color ?? null, icon ?? null, sortOrder ?? null, groupName !== undefined, groupName ?? null]
  );
  if (!rows.length) throw new HttpError(404, 'Category not found');

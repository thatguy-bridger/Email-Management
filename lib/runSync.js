...
export async function runAccountSync(account) {
  ...
  try {
    const { messages, maxUid, syncMailbox, backfillBeforeUid, backfillComplete, totalUidEstimate } =
    ...
    await query(
      `UPDATE accounts SET
         ...
         backfill_total_estimate = $6, last_synced_at = now(), sync_status = 'idle', sync_error = NULL
       WHERE id = $1`,
      [account.id, Math.max(maxUid, account.last_uid), syncMailbox, backfillBeforeUid, backfillComplete, totalUidEstimate]
    );

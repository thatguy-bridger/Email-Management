...
export async function importGmailFilters(user, req, res) {
  ...
  const { rows: existingCats } = await query('SELECT id, name, group_name FROM categories WHERE user_id = $1', [
    user.id,
  ]);
  const categoryCache = new Map();
  for (const c of existingCats) {
    const path = c.group_name ? `${c.group_name}/${c.name}` : c.name;
    categoryCache.set(path.toLowerCase(), c.id);
  }
  const startingCategoryCount = categoryCache.size;
  ...
  let paletteIdx = categoryCache.size;

  async function getOrCreateCategory(path) {
    const key = path.toLowerCase();
    if (categoryCache.has(key)) return categoryCache.get(key);
    const slash = path.indexOf('/');
    const groupName = slash === -1 ? null : path.slice(0, slash);
    ...
    await query(
      ...
      [id, user.id, name, color, nextSort++, groupName]
    );
  ...
  res.status(200).json({
    ...
    categoriesCreated: categoryCache.size - startingCategoryCount,
    rulesCreated,
    duplicateRulesSkipped,
    skipped,
    skippedDetails,

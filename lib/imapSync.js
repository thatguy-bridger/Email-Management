...
// Each individual chunk request is kept at the size that's always safely
// finished a single sync call before (proven not to risk the serverless
// function's 60s hard limit even for large HTML messages), but rather than
// stopping after just one chunk, syncAccount keeps requesting further-back
// chunks in a loop until BACKFILL_TIME_BUDGET_MS is spent -- so a call
// against a fast connection with modestly-sized messages gets through many
// times more mail instead of leaving most of a 60s budget unused. Because
// the deadline is only ever checked *between* whole chunks (never used to
// cut one short mid-fetch), the resumption watermark only ever advances by
// fully-completed chunks -- no partial-chunk bookkeeping, no way to skip
// mail by bailing at the wrong moment.
const BACKFILL_CHUNK_SIZE = 150;
// Checked only *between* chunks, not preemptively during one -- so this
// needs real margin below the 60s hard limit, not just enough to cover the
// typical case, since starting one chunk too many is exactly what would
// blow the budget and lose the whole call's unpersisted work (nothing is
// saved until syncAccount returns and runSync.js writes it out). 30s leaves
// room for connect/list/incremental-fetch beforehand, one more
// slower-than-usual chunk in flight when the check lands, and the (now
// batched, but still not instant) DB inserts afterward.
...
// new mail, plus (separately) as many time-boxed chunks of older mail as
// fit in one call for historical backfill. The sync source (INBOX vs. a
// special-use All Mail/Archive folder) can change between calls -- most commonly the very
    ...
    try {
      ...
      if (account.sync_mailbox !== mailboxInfo.path) {
        ...
        if (!account.backfill_complete && account.backfill_before_uid > 1) {
          ...
          let cursor = account.backfill_before_uid;
          while (cursor > 1 && Date.now() < deadline) {
            const chunkStart = Math.max(1, cursor - BACKFILL_CHUNK_SIZE);
            results = results.concat(await fetchRange(client, `${chunkStart}:${cursor - 1}`, mailboxInfo));
            cursor = chunkStart;
          }
          newBackfillBeforeUid = cursor;
          newBackfillComplete = cursor <= 1;
        }
      }
      ...
      return {
        ...
        // Rough "how much mail exists in total" estimate for the progress
        // readout -- see the column comment in schema.sql for why this is
        // an estimate, not an exact count.
        totalUidEstimate: uidNext - 1,
      };
    } finally {
      lock.release();
    }
